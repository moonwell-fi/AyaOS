...
AyaOS is a lightweight wrapper around ElizaOS for creating autonomous AI agents. The runtime is self-contained: it does not authenticate with, provision through, or connect to an AyaOS-hosted service. Model providers and platform integrations are opt-in and use only the credentials and endpoints you configure.
...
When you start an agent for the first time, AyaOS creates a local profile:

1. A UUID and generic ElizaOS character are generated locally
2. The character is stored as `character.json` in the configured data directory
3. Future starts reuse the same local profile

No AyaOS account, token, or network connection is required. You can also pass a character directly with `new Agent({ character })`, or set `AGENT_NAME` and `AGENT_PURPOSE`.

To use the bundled OpenAI-compatible model plugin, configure your own provider:

```bash
OPENAI_API_KEY=your_api_key
# Optional for local or alternative OpenAI-compatible providers:
...
```typescript
import { Agent, Action } from '@moonwell-fi/ayaos'
...
```typescript
import { Agent } from '@moonwell-fi/ayaos'
...
Provides local EVM wallet functionality for blockchain interactions. Set one of `EVM_WALLET_PRIVATE_KEY`, `WALLET_PRIVATE_KEY`, or `XMTP_WALLET_PRIVATE_KEY`; keys are never fetched from AyaOS:
...
```typescript
import { Agent, Service, ServiceType } from '@moonwell-fi/ayaos'
...
```typescript
import { Agent, Provider } from '@moonwell-fi/ayaos'
...
Language models are provided by the OpenAI-compatible endpoint you configure. AyaOS does not supply a hosted default model or proxy.
...
1. **Using a local wallet**: AyaOS uses a locally configured EVM private key
2. **Using a specific private key**: Provide a dedicated private key for XMTP

#### Configuration

**Option 1: Using a Local Wallet (Recommended)**

Configure a private key in the agent environment:

```bash
EVM_WALLET_PRIVATE_KEY=0x1234567890abcdef...
```
